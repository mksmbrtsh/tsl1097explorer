...
public class BluetoothService {
		...
		public ConnectThread(BluetoothDevice device) {
			...
			try {
				if (BluetoothService.this.isAndroid)
					tmp = device
							.createRfcommSocketToServiceRecord(UUID_ANDROID_DEVICE);
				else
					tmp = device
	...
	private class ConnectedThread extends Thread {
			...
			while (true) {
				try {
					...
					BluetoothService.this
							.start(BluetoothService.this.isAndroid);
					break;
				}
			}
		}
		...
		public void write(byte[] buffer) {
			try {/*
				 * byte[] buffer2 = new byte[buffer.length + 2]; for(int i = 0 ;
				 * i < buffer.length ; i++) buffer2[i] = buffer[i];
				 * buffer2[buffer2.length - 2] = 0x0A; buffer2[buffer2.length -
				 * 1] = 0x0D;
				 */
				mmOutStream.write(buffer);
				// Share the sent message back to the UI Activity
				mHandler.obtainMessage(BluetoothState.MESSAGE_WRITE, -1, -1,
						buffer).sendToTarget();
			} catch (IOException e) {
			}
		}
